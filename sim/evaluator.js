...
function evaluate(team,names,N){
  ...
  const dotSpread=Math.max(...v)/Math.max(0.01,Math.min(...v));
  ...
  const variety=Math.min(1,(dotSpread-1)/1.6);
  ...
  return {names,avg:+avg.toFixed(2),dotSpread:+dotSpread.toFixed(2),
