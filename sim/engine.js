...
const ARMOR_PER_LAYER=14,OFF_ELEMENT_SHRED=.2,ARMOR_MITIGATION=.55,VULN_BONUS=.35,BREAK_DELAY=.35;
const ELS=['order','chaos','growth','void','decay','energy'];
const AILS=['burn','poison','frost','dotTaken'];

// ---- enemy archetypes: each one is a different problem to solve ----
const FOES={
  swarm:    {hp:.55,dmg:.5, armorLayers:1,speed:96, note:'numerous and disposable'},
  bruiser:  {hp:1.7, dmg:1.1,armorLayers:2,speed:74, note:'slow, heavy, high health'},
  skirmisher:{hp:.7,dmg:.9, armorLayers:1,speed:126,backline:1,note:'fast, ignores the front row'},
  armored:   {hp:1.15,dmg:.5, armorLayers:7,speed:82, protectAlly:0,dotTaken:2.1,note:'enormous shields \u2014 decay walks straight past them'},
  cleanser: {hp:1.1, dmg:.8, armorLayers:2,speed:104,cleanse:.45,note:'sheds some of what you put on it'},
  splitter: {hp:1.2, dmg:.7, armorLayers:1,speed:92, split:1,note:'leaves one swarm behind when it falls'},
  harrier:  {hp:.9,  dmg:.32,armorLayers:1,speed:118,multiHit:3,note:'many small hits, chews through barriers'},
  buffer:   {hp:1.2, dmg:.5, armorLayers:2,speed:88, buffAllyDmg:.14,note:'strengthens every other enemy'},
  reflector:{hp:0.95,dmg:.7, armorLayers:2,speed:90, thorns:.34,note:'answers each hit with a little of its own'}
};
...
const DEFAULTS={
  ...
  marks:0, armorShred:0, harmony:0, upgrades:0,
  healer:1, healAmt:26
};
...
function buildCrew(P){
  ...
  for(let i=0;i<n;i++){
    const c={id:'a'+i,side:'ally',e:ELS[i%6],
      speed:88+((i*7)%P.speedSpread)+ (i%2?6:0),
      dmg:Math.round(P.dmg*(1+0.42*(P.upgrades/Math.max(1,n)))),
      hp:110,max:110,alive:true};
    if(i<P.aoeUnits){ c.aoe=1; c.armorShred=P.armorShred||Math.round(P.dmg*0.7); }
    if(P.dotUnits>0 && i<Math.min(n-1,P.dotUnits)) c.appliesDot=AILS[i%P.dotTypes];
    if(P.detonator && i===n-2) { c.detonate=1; c.appliesDot=null; }
    if(P.reactors && i>=n-1-P.reactors && i<n-1) { c.procOn='ally'; }
...
function fight(P,E,seed){
  ...
  const U=buildCrew(P).map(c=>({...c,av:10000/c.speed,hp:E.crewHp,max:E.crewHp}));
  const roster=[];
  // mix is a list of [archetype, count]
  (E.mix||[['swarm',Math.round(pool*0.8)],['bruiser',Math.round(pool*0.2)]])
    .forEach(([k,n])=>{ const a=FOES[k];
      for(let i=0;i<n;i++) roster.push({kind:k,elite:a.hp>=1.2,
        ...
        shL:a.armorLayers, speed:a.speed, spec:a}); });
  ...
  function spawn(){ while(U.filter(u=>u.side==='foe'&&u.alive).length<field&&spawned<roster.length){
    ...
    U.push({id:'f'+spawned,side:'foe',elite:p.elite,kind:p.kind,spec:p.spec,
      ...
      speed:p.speed,dmg:p.dmg,hp:p.hp,max:p.hp,av:(10000/p.speed)*(.4+rnd()*.7),
      alive:true,shL:p.shL,shC:ARMOR_PER_LAYER,broken:false,vuln:0,dotStacks:{}}); spawned++; } }
  spawn();
  const alive=()=>U.filter(u=>u.alive);
  const foes=()=>U.filter(u=>u.side==='foe'&&u.alive);
  const isW=(t,e)=>t.weak.indexOf(e)>=0;
  const types=t=>t.dotStacks?AILS.filter(k=>t.dotStacks[k]>0).length:0;
  const dotPerHit=t=>t.dotStacks?AILS.reduce((a,k)=>a+(t.dotStacks[k]||0),0):0;

  let av=0,round=1,g=0,overheal=null;
  // instrumentation
  ...
  function ailBonus(t){ return ((t.dotStacks&&t.dotStacks.burn)||0)*0.07; }
  function hurt(s,t,a){
    if(!t||!t.alive) return 0;
    let soak=(t.side==='foe'&&!t.broken&&t.shL>0)?ARMOR_MITIGATION:1;
    if(t.side==='foe'){
      const w=foes().find(f=>f.spec&&f.spec.protectAlly&&f!==t);
      if(w) soak*= (1-w.spec.protectAlly);
    }
    let d=Math.round(a*(1+(t.vuln>0?VULN_BONUS:0)+ailBonus(t))*soak);
    ...
    if(t.hp<=0&&t.alive){ t.alive=false;
      if(t.spec&&t.spec.split&&splitsLeft>0){
        ...
        for(let q=0;q<t.spec.split;q++){
          U.push({id:'s'+(splitId++),side:'foe',kind:'swarm',spec:FOES.swarm,
            weak:t.weak.slice(),e:t.weak[0],speed:96,dmg:Math.round(t.dmg*0.5),
            hp:Math.round(E.hpBase*0.4),max:Math.round(E.hpBase*0.4),
            av:(10000/96)*0.5,alive:true,shL:1,shC:ARMOR_PER_LAYER,broken:false,vuln:0,dotStacks:{}});
        } splits+=t.spec.split; } }
    return d; }
  function shredT(s,t,a){ if(!t||t.broken||t.shL<=0) return false;
    let p=a*(isW(t,s.e)?1:OFF_ELEMENT_SHRED);
    while(p>0&&t.shL>0){ if(p>=t.shC){p-=t.shC;t.shL--;t.shC=t.shL>0?ARMOR_PER_LAYER:0} else {t.shC-=p;p=0} }
    if(t.shL<=0){t.broken=true;t.vuln=1;t.av+=(10000/t.speed)*BREAK_DELAY;breaks++;return true;}
    return false; }
  function fireAils(t,boost,clear){
    let total=0,fired=0;
    AILS.forEach(k=>{
      const n=(t.dotStacks&&t.dotStacks[k])||0; if(!n) return;
      const vuln=(t.spec&&t.spec.dotTaken)||1;
      ...
      fired++;
      t.dotStacks[k]=clear?0:Math.max(0,n-1);
    });
    if(fired&&P.siphon){
  ...
  function cascade(src,cur,depth){
    ...
    U.filter(r=>r.side==='ally'&&r.alive&&r.procOn&&r.id!==src.id).forEach(r=>{
  ...
  while(g++<40000){
    const al=alive();
    if(spawned>=roster.length&&!foes().length){overheal='win';break}
    if(!al.some(u=>u.side==='ally')){overheal='wipe';break}
    ...
    if(nr>round){ perRound.push(roundDmg); roundDmg=0; round=nr;
      if(round>E.roundCap){overheal='timeout';break} }
    U.forEach(u=>{ if(u.side==='ally') u._b=0; });
    if(act.side==='foe'){
      fireAils(act,0,false);
      if(!act.alive){act.av=10000/act.speed;spawn();continue;}
      if(act.broken){act.broken=false;act.vuln=0;act.shL=act.elite?E.eliteShl:E.chaffShl;act.shC=ARMOR_PER_LAYER;
        act.av=10000/act.speed;continue;}
      const kind=act.spec||{};
      // an buffer makes everything else hit harder
      if(kind.buffAllyDmg){ foes().forEach(f=>{ if(f!==act) f.dmg=Math.round(f.dmg*(1+kind.buffAllyDmg)); });
        act.av=10000/act.speed; continue; }
      // a cleanser wipes the ailments off the whole board
      if(kind.cleanse){ let wiped=0;
        foes().forEach(f=>{ AILS.forEach(k=>{ const n=(f.dotStacks&&f.dotStacks[k])||0;
          if(n){ const off=Math.max(1,Math.round(n*kind.cleanse)); f.dotStacks[k]=n-off; wiped+=off; } }); });
        if(wiped){ cleansed+=wiped; act.av=10000/act.speed; continue; } }
      const pool3=alive().filter(u=>u.side==='ally');
      const shots=kind.multiHit||1;
    ...
    if(act.detonate){
      const loaded=fs.filter(t=>dotPerHit(t)>0);
      if(loaded.length>=2||loaded.some(t=>dotPerHit(t)>=5)){
        let burst=0; loaded.forEach(t=>burst+=fireAils(t,P.detBoost,true));
        biggest=Math.max(biggest,burst); dets++;
        act.av=10000/act.speed; spawn(); continue; } }
    let t=null,bw=-1e9;
    fs.forEach(x=>{ const sl=x.broken?0:((x.shL-1)*ARMOR_PER_LAYER+x.shC);
      const eff=act.dmg*(1+ailBonus(x))*((!x.broken&&sl>0)?ARMOR_MITIGATION:1);
      let sc=-(sl/Math.max(1,act.dmg*(isW(x,act.e)?1:OFF_ELEMENT_SHRED))+x.hp/Math.max(1,eff))*.6;
      if(act.appliesDot&&dotPerHit(x)<P.dotCap) sc+=.8;
      if(x.vuln>0)sc+=.7; if(sc>bw){bw=sc;t=x} });
    if(!t) t=fs[0];
    const targets=act.aoe?fs.slice():[t];
    targets.forEach(x=>{
      shredT(act,x,act.aoe?(act.armorShred||act.dmg):act.dmg);
      hurt(act,x,act.dmg*(act.aoe?P.aoeRatio:1));
      if(act.appliesDot) x.dotStacks[act.appliesDot]=Math.min(P.dotCap,(x.dotStacks[act.appliesDot]||0)+P.dotApply);
    });
  ...
  const dotSpread=Math.sqrt(shares.reduce((a,x)=>a+(x-mean)**2,0)/shares.length)/mean;
  ...
  return {overheal,round,events,biggest,dotDmg,healed,ticks,follows,breaks,dets,
          cleansed,thornDmg,splits,
          evenness:Math.max(0,1-dotSpread), escalation:early?late/early:1};
}
...
function evaluate(P,encounters,N){
  ...
  encounters.forEach(E=>{
    for(let s=1;s<=N;s++){
      ...
      agg.n++;
      if(f.overheal==='win')agg.win++; else if(f.overheal==='wipe')agg.wipe++; else agg.timeout++;
