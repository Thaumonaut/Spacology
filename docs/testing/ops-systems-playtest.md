...
- Five two-item gear recipes. Drag loose gear onto another item to preview and confirm a combination, or use Forge. Inputs are consumed only when the recipe is valid and the output is unowned. Crafted effects are applied by the live battle loader.
- Crew upgrades cost 8 scrap + 1 crystal per rank, capped at rank III. Each rank adds 15% base health and 10% base attack.
- Attunements cost 12 scrap + 1 crystal and recruit a chosen unowned character of that element. Only elements represented by playable crew are offered.
