...
assert.equal(F.position('Arunima'),'field');assert.equal(F.position('Ivara'),'support');
for(const n of ['Hanae','Quill','Maul'])assert.equal(F.position(n),'both');
const s={field:['Daven','Hanae','Ivara'],support:['Arunima','Roonie','Veska'],reserve:[],equipped:{Arunima:['Bore Bit']}};
const before=names(s);F.normalize(s);assert.deepEqual(names(s),before);assert(s.field.includes('Arunima'));assert(s.support.includes('Ivara'));assert.equal(s.field[0],'Daven');assert.deepEqual(s.equipped,{Arunima:['Bore Bit']});assert.deepEqual(F.normalize(s),[]);
